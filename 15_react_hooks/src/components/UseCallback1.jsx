...
export default function UseCallback1() {
  ...
  //useEffect에서 의존성 배열로 func1에 변경사항이 생겨도 내부 로직을 실행하도록 작성
  //매번 useEffect의 콜백 함수
  const func1 = () => {
  ...
  //메모이제이션 한 콜백 함수를 useCallback hook의 첫 번째 인자로 전달한 콜백 함수를 다시 메모제이션
  const func2 = useCallback(() => {
    console.log(`number state :${number} `);
  }, [number]);

  useEffect(() => {
    console.log("func1 함수로 변경!!!!");
  }, [func1]);

  useEffect(() => {
    console.log("func2 함수로 변경!!!!");
  }, [func2]);
  return (
    <>
...
// }, [fetchData]);
// //처음 컴포넌트 mount시 fetchData 함수 실행과 동시에 메모리에 콜백 함수 자체각 저장됨
